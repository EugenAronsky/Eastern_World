...
const GridLine = ({list}) =>{
    ...
    function creatGridElemnt(){
        ...
        const blur = (element) => {
            if(focusElement === element || focusElement.parentElement === element){
        ...
        return elements.map((info, i)=>
            {
                return(
                    <div name={0} onBlur={(element)=>blur(element.target)} onClick={(element)=>Check_sate(element.target, info)} tabIndex={tabIndex} key={i} style={{gridColumn: grid[i], backgroundImage: "url(" + info.previewImage.fields.file.url + ")"}}>
